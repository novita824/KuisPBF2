  ...
  return (
    <div className="relative p-5 overflow-x-auto">
      <div className="flex items-center justify-between mb-11">
        <h1 className="text-5xl font-semibold text-white">List Rekening</h1>
        <Link to="/banks/create" className="btn text-black bg-[#e0b1cb] hover:bg-[#be95c4]">Create New</Link>
      </div>
        ...
        <table className="table w-full text-center">
          <thead>
            <tr>
              ...
              <th className="bg-[#231942] text-white">Nomor Rekening</th>
          ...
          <tbody>
            ...
            </tr> : banks.map((bnk, index) => (
              <tr key={index}>
                <th className="bg-[#FFFBEB]">{index + 1}</th>
                <td className="bg-[#FFFBEB]">{bnk.name}</td>
                <td className="bg-[#FFFBEB]">{bnk.nomorRekening}</td>
                <td className="bg-[#FFFBEB]">
                  <Link
                    to={`/banks/edit/${bnk.id}`}
                    className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                  >
                    Edit
                  </Link>
                  <button
                    onClick={() => handleDelete(bnk.id)}
